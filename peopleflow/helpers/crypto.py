...
# Huge thanks to https://gist.github.com/marcoslin/8026990
...
def encrypt_string(to_encrypt, key):
...
def nbyte_digest(string, n):
    dig = hashlib.sha1(string).hexdigest()

    if len(dig) >= n:
        return dig[0:n]
    else:
        return dig + nbyte_digest(dig, n - len(dig))
